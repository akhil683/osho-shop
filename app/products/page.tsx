...
const ProductSkeleton = ({ viewMode }: { viewMode: "grid" | "list" }) => (
  <Card
    className={`group overflow-hidden animate-pulse ${
  ...
  >
    ...
    >
      <div
        className={`bg-gray-200 ${viewMode === "list" ? "w-48 h-32" : "w-full h-48"}`}
      />
    </CardHeader>
    <div
          ...
          {isError ? (
            ...
            >
              {filteredAndSortedProducts.map((product: TProduct) => (
                <Card
                  key={product.id || product.imageUrl} // Use id if available
